"""
Excel/CSV/Parquet SQL Workbench - High-Performance Memory-Optimized Version
Uses native column tables to maximize query processing execution speeds.
"""
...
# ==========================================================
st.set_page_config(page_title="Excel SQL Workbench", layout="wide")

if "con" not in st.session_state:
    con = duckdb.connect()
    # Allocate dedicated processing sandbox limits for analytical streams
...
def generate_table_name(index: int) -> str:
    """Generate sequential database table names: dataA, dataB, ..., dataZ"""
...
def get_table_stats(table_name: str) -> dict:
    """Safely extracts profile statistics from catalog tables"""
    try:
...
def register_file(file_name: str, file_path: str, file_size: int) -> Tuple[bool, list]:
    ...
    try:
        if lower.endswith(".csv"):
            ...
            # CRITICAL OPTIMIZATION: Created as a static TABLE instead of a VIEW.
            # This pulls raw bytes into DuckDB's lightning-fast columnar indexing matrix once.
            con.execute(f"""
                CREATE TABLE {table_name} AS
                SELECT * FROM read_csv_auto('{file_path}', parallel=true)
            """)
            
            st.session_state.datasets[table_name] = {
                "filename": file_name, "path": file_path, "uploaded_at": datetime.now(), "size": file_size
            }
            ...
            # OPTIMIZATION: Do the exact same thing for Parquet source columns
            con.execute(f"""
                CREATE TABLE {table_name} AS
            ...
            for sheet in xl.sheet_names:
                ...
                st.session_state.datasets[table_name] = {
                    "filename": f"{file_name} ({sheet})", "path": file_path, "uploaded_at": datetime.now(), "size": file_size
                }
                registered_tables.append(table_name)
        else:
            return False, [f"Unsupported type: {file_name}"]
...
with st.sidebar:
    ...
    result_limit = st.slider(
        "Max preview rows to load in UI",
    ...
    if upload_method == "Direct File Path":
        ...
        uploaded_files = st.file_uploader(
            "Upload files (Max 5GB)", type=["csv", "parquet", "xlsx", "xls"], accept_multiple_files=True
        )
...
query = st.text_area("Enter SQL (Standard DuckDB/PostgreSQL syntax supported)", value=default_query, height=180)
...
if result is not None:
    ...
    with ex1:
        ...
        st.download_button(
            "📥 Download CSV (Preview)", csv_data, 
            f"query_preview_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True
        )
    ...
    with ex2:
        try:
            ...
            st.download_button(
                "📥 Download Excel (Preview)", data=buffer.getvalue(),
                file_name=f"query_preview_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
    ...
    with ex3:
        ...
        st.download_button(
            "📥 Download JSON (Preview)", json_data,
            f"query_preview_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "application/json", use_container_width=True
        )

    # High-Speed Native Engine Direct-To-Disk Export
    st.markdown("---")
    ...
    export_path_input = exp_col1.text_input(
        "Target Absolute Path for Full Dataset Save", 
        placeholder="C:/Users/Desktop/complete_output.csv", 
        key="native_export_field"
    )
